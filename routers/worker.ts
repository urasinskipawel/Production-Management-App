...
workerRouter
  ...
  .get('/panel/:workerFirstName/:workerLastname', async (req: Request, res: Response): Promise<void> => {
    const { workerFirstName } = req.params;
    const { workerLastname } = req.params;
    ...
    const currentTime = handlebarsHelpers.getCurrentTime();
    const finishTime = handlebarsHelpers.getCurrentTime();
